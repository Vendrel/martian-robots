...
const s={rover:'curiosity',images:[],yaw:0,pitch:0,fov:MAX_FOV_DEGREES*D,pointer:null,drag:false,x:0,y:0,sy:0,sp:0,ready:0,selection:null,selectionDraft:null};
...
function activeImages(){const ids=s.selection?.ids||s.selectionDraft?.ids;return ids?.length?s.images.filter(image=>ids.includes(image.imageid)):s.images}
...
function encodeRay(vector){return vector.map(value=>value.toFixed(6)).join(',')}
function decodeRay(value){const vector=String(value||'').split(',').map(Number);return vector.length===3&&vector.every(Number.isFinite)&&Math.hypot(...vector)>0?norm(vector):null}
function shareParams(){const params=new URLSearchParams();params.set('rover',s.rover);params.set('sol',$('#solSelect').value);params.set('yaw',(s.yaw/D).toFixed(3));params.set('pitch',(s.pitch/D).toFixed(3));params.set('fov',(s.fov/D).toFixed(3));params.set('theme',document.documentElement.dataset.theme);if(s.selection){params.set('selectionCenter',encodeRay(s.selection.center));params.set('selectionEdge',encodeRay(s.selection.edge));}else if(!$('#imagePanel').hidden)params.set('image',$('#imageTitle').textContent);return params;}
...
function restoreShareUrl(){const params=new URLSearchParams(location.hash.slice(1));if(!params.size)return;restoringShare=true;const rover=params.get('rover');if(rovers[rover]){s.rover=rover;$('#roverSelect').value=rover;sols(rovers[rover].latest)}const sol=params.get('sol');if(sol&&$('#solSelect').querySelector(`option[value="${sol}"]`))$('#solSelect').value=sol;for(const [key,field]of [['yaw','yaw'],['pitch','pitch'],['fov','fov']]){const value=Number(params.get(key));if(Number.isFinite(value))s[field]=value*D}const theme=params.get('theme');if(theme==='dark'||theme==='light'){document.documentElement.dataset.theme=theme;$('#themeToggle').textContent=theme==='dark'?'☾':'☼'}const center=decodeRay(params.get('selectionCenter')),edge=decodeRay(params.get('selectionEdge'));pendingSharedSelection=center&&edge?{center,edge}:null;pendingSharedImage=pendingSharedSelection?null:params.get('image');$('#loadButton').click();let attempts=0;const reveal=setInterval(()=>{const image=s.images.find(item=>item.imageid===pendingSharedImage);if(image){panel(image);pendingSharedImage=null;clearInterval(reveal);restoringShare=false;scheduleShareUrl()}else if(!pendingSharedImage||++attempts>250){pendingSharedImage=null;clearInterval(reveal);restoringShare=false;scheduleShareUrl()}},100);}
...
function clearSelection(){s.selection=null;s.selectionDraft=null;$('#imagePanel').classList.remove('selectionMode');$('#imagePanel .eyebrow').textContent=t.imagePanelEyebrow||'IMAGE INFO PANEL';$('#closePanel').hidden=false;panel(null);render();}
const normalPanel=panel;
panel=(image)=>{if(s.selection){if(!image){$('#imagePanel').hidden=true;return;}showSelectionPanel();scheduleShareUrl();return;}$('#imagePanel').classList.remove('selectionMode');$('#imagePanel .eyebrow').textContent=t.imagePanelEyebrow||'IMAGE INFO PANEL';$('#closePanel').hidden=false;normalPanel(image);};
...
function finishSelection(event){const cap=selectionCap(),ids=s.selectionDraft?.ids||selectedImageIds(cap);s.selectionDraft=null;wrap.releasePointerCapture?.(event.pointerId);if(!ids.length){clearSelection();return;}s.selection={center:cap.center,edge:cap.edge,ids};showSelectionPanel();render();}
function restorePendingSelection(){if(!pendingSharedSelection||!s.images.length)return;const shared=pendingSharedSelection;pendingSharedSelection=null;const cosine=Math.max(-1,Math.min(1,dot(shared.center,shared.edge))),cap={...shared,radius:Math.acos(cosine)},ids=selectedImageIds(cap);if(!ids.length)return;s.selection={...shared,ids};showSelectionPanel();render();}
