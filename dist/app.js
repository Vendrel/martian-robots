...
function showSelectionPanel(){const count=s.selection?.ids.length||0;$('#imagePanel .eyebrow').textContent=t.selectionPanelTitle||'IMAGE SELECTION';$('#imageTitle').textContent=(t.imagesSelected||'{count} IMAGES SELECTED').replace('{count}',count);$('#closePanel').hidden=true;$('#panelImage').hidden=true;$('#metadataList').hidden=true;$('#sourceLink').hidden=true;$('#downloadAllButton').textContent=t.downloadSelected||'Download selected';$('#imagePanel').hidden=false;}
function clearSelection(){s.selection=null;s.selectionDraft=null;$('#imagePanel .eyebrow').textContent=t.imagePanelEyebrow||'IMAGE INFO PANEL';$('#closePanel').hidden=false;$('#panelImage').hidden=false;$('#metadataList').hidden=false;$('#sourceLink').hidden=false;panel(null);render();}
const normalPanel=panel;
panel=(image)=>{if(s.selection){if(!image){$('#imagePanel').hidden=true;return;}showSelectionPanel();scheduleShareUrl();return;}$('#imagePanel .eyebrow').textContent=t.imagePanelEyebrow||'IMAGE INFO PANEL';$('#closePanel').hidden=false;$('#panelImage').hidden=false;$('#metadataList').hidden=false;$('#sourceLink').hidden=false;normalPanel(image);};
