...
async function downloadAll(){const button=$('#downloadAllButton'),images=[...s.images];if(!images.length||button.disabled)return;button.disabled=true;const original=t.downloadAll||'Download all';let failed=0;try{saveBlob(new Blob([JSON.stringify(exportManifest(),null,2)],{type:'application/json'}),`mars-rover-360-${s.rover}-sol-${$('#solSelect').value}-manifest.json`);await sleep(DOWNLOAD_DELAY_MS);for(let index=0;index<images.length;index++){button.textContent=(t.downloading||'Downloading {current}/{total}…').replace('{current}',index+1).replace('{total}',images.length);try{await saveImage(images[index],index)}catch(error){failed++;debug('Image export blocked', `${downloadName(images[index],index)}: ${error.name||'unknown error'}`)}if(index<images.length-1)await sleep(DOWNLOAD_DELAY_MS);}}finally{button.disabled=false;button.textContent=failed?(t.downloadFailed||'Image download unavailable ({count})').replace('{count}',failed):original;}}
